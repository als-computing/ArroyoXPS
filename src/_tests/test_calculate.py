...
def load_test_array(file_name="./src/_tests/test_array_300_1131.npy"): 
    """
    Helper function to load the test array, each row of the test array is a integrated frame already
    """
    file_path = Path(file_name)
    array = np.load(file_path)
    assert array.shape[0] == 300, f"Expected test array with 300 rows, got {array.shape[0]}"
    return array

def load_test_raw_array(file_name="./src/_tests/test_raw_frame_269_1131.npy"):
    """
    ...
    assert duplicated_array.shape[0] == 100, f"Expected test raw array with 300 frames, got {array.shape[0]}"
    return duplicated_array

def test_peak_detection():
    """Test the peak detection functionality."""
    array = load_test_array()
    ...
    assert peak_locations[0] in range(300, 400), f"First peak at unexpected location {peak_locations[0]}"
    assert peak_locations[1] in range(600, 700), f"Second peak at unexpected location {peak_locations[1]}"
...
def test_fft_items():
    ...
    assert vfft.shape == array.shape, "vfft shape mismatch"
    assert len(sum.shape) == 1, "sum should be a 1D array"
    assert ifft.shape == array.shape, "ifft shape mismatch"

def test_calculate():
    """Test the main calculate function."""
    # load the test raw array, which is before being integrated
    array = load_test_raw_array()
   
    calculated_item = calculate(array, repeat_factor = 20)

    assert len(calculated_item.integrated_frame.shape) == 1, "integrated_frame should be 1D"
    assert isinstance(calculated_item.detected_peaks, np.ndarray), "detected_peaks should be an ndarray"
