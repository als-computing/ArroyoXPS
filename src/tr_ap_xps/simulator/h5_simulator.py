...
class H5LabViewSimulator:
    def __init__(self, file: str, scan: str, zmq_socket: zmq.Socket, scan_pause: int, repeat_scans: int):
        self.file = file
        self.scan = scan
...
def start(
    ...
    log_level: str ="DEBUG",
    scan_pause: int = 5,
    repeat_scans:int = typer.Option(50, help="Number of times to repeat data from a scan as a full scan")
) -> None:
    setup_logger(logger)
