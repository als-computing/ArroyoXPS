...
export default function FormRow({inputItem={}, handleInputChange=()=>{}, id='', labelStyles='', inputStyles='', layout='horizontal'}) {
    ...
    const renderInput = () => {
        switch (inputItem.type) {
            ...
            case type.file:
                return <FormInputString handleInputChange={handleInputChange} value={inputItem.value} item={inputItem} inputStyles={inputStyles} id={id}/>
            case type.boolean:
                return <FormInputBoolean handleInputChange={handleInputChange} value={inputItem.value} item={inputItem} inputStyles={inputStyles} id={id}/>
            default:
    ...
    if (layout === 'horizontal') {
        return (
            <li className='flex'>
                <label className='w-3/5 flex-shrink-0 text-right pr-3'>{inputItem.label}</label>
                {renderInput()}
            </li>
        )
    }
