...
"""
Prepare a detector for acquisition process, starting acquisition, getting preview and saving images
...
def preview(serverurl, ntrig=1):
    ...
    for i in range(ntrig):
        ...
        response = get_request(url=serverurl + '/measurement/image')
        image = Image.open(BytesIO(response.content))
        # Show the data in the image
        image.show()
        # Save a preview image
        image.save(pathlib.Path(os.path.join(os.getcwd(), 'data', 'test-preview{}.png'.format(i))))
...
if __name__ == '__main__':
    ...
    numTriggers = 5
    init_acquisition(serverurl, detectorConfig, numTriggers, 1, 0.10)
    ...
    # Rotate image
    response = get_request(url=serverurl + '/detector/layout/rotate?reset&direction=right')
    # response = get_request(url=serverurl + '/detector/layout/rotate?reset&direction=left')
    # response = get_request(url=serverurl + '/detector/layout/rotate?reset&direction=180')
    # response = get_request(url=serverurl + '/detector/layout/rotate?reset&flip=horizontal')
    # response = get_request(url=serverurl + '/detector/layout/rotate?reset&flip=vertical')
    # response = get_request(url=serverurl + '/detector/layout/rotate?reset&direction=right&flip=horizontal')

    # Running acquisition process and preview
    simple_acquisition(serverurl)
    preview(serverurl, numTriggers)
